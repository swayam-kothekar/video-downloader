...
class StorageService {
  ...
  String generateFileName(String title, String quality, String extension) {
    // Remove all non-alphanumeric, non-space, non-underscore, non-hyphen characters
    final safeTitle = title
        .replaceAll(RegExp(r'[^a-zA-Z0-9\s_-]'), '')
        .replaceAll(RegExp(r'\s+'), ' ')
        .trim();

    // Limit filename length to 60 to prevent long paths/SQLite insert errors
    final maxLength = 60;
  ...
  /// Save download logs (keeps only last 50)
  Future<void> saveDownloadLogs(List<DownloadLog> logs) async {
    try {
      ...
      final logsToSave = logs.length > 50 ? logs.sublist(0, 50) : logs;
